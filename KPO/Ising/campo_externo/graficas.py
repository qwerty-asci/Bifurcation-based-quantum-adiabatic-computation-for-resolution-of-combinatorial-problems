...
for nombre in os.listdir("resultados"):
    ...
    plt.xlim(0.0,tlist[np.where(tlist<=600,True,False)][-1])
    plt.tight_layout()
    ...
    plt.ylabel("$E_{ISING}$",fontsize=15)
    plt.title("Energy levels of the Ising model",fontsize=20)
    for j in range(2**n_oscillators):
        ...
        try:
            ...
            plt.plot([0.0,1.0],[aux,aux],color="#000000")
            plt.text(0.01,aux+0.15,r"$\left |"+string+r"\right>$+equiv.")

        E.append(aux)

    plt.xlim(0.0,1.0)
    plt.savefig("graficas/energy_levels_"+str(K)+"_pf_"+str(pf)+"_xi_"+str(xi)+"_tmax_"+str(tmax)+"_n_osciladores_"+str(n_oscillators)+"_N_"+str(N)+".png",format="png",bbox_inches="tight")
