...
def medidas_bloque2(fila,estado,archivo,nombres):
    ...
    for q in fila:
        ...
        for i in range(2**n_oscillators):
            ...
            archivo.to_csv("Ising_sin_campo_externo_K_"+str(K)+"_pf_"+str(pf)+"_xi_"+str(xi)+"_tmax_"+str(tmax)+"_n_osciladores_"+str(n_oscillators)+"_N_"+str(N)+".csv")
...
try:
    ...
    for j in tqdm(range(int(samples/bloques))):
        ...
        result=sesolve([H0,[Ht,f]],est,tlist[contador*bloques:(contador+1)*bloques],options={
    ...
    for trab in futures.as_completed(trabajos):
        ...
        suma+=trab.result()
    progress_bar.close()
    resultados.to_csv("Ising_sin_campo_externo_K_"+str(K)+"_pf_"+str(pf)+"_xi_"+str(xi)+"_tmax_"+str(tmax)+"_n_osciladores_"+str(n_oscillators)+"_N_"+str(N)+".csv")
