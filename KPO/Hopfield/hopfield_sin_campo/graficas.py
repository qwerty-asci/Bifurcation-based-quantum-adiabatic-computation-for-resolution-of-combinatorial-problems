...
name="Hopfield_sin_campo_externo_"
...
#     plt.plot(-f(tlist[1:])/K,df["Prob_conf_NL"][str(0)].loc[1:],label=str(s),color="blue",linewidth=3)
#
#     plt.title(r"Fidelity respect to the ground state of $H_{NL}$",fontsize=16)
#
#
#     plt.ylim(0,1.0)
#     plt.xlim(0.0,-f(tlist[-1]))
...
#
    ...
    plt.title("Success probability of each configuration",fontsize=25)
    plt.yticks([0.1*i for i in range(11)],fontsize=15)
    plt.xticks([20*i for i in range(100)],fontsize=15)
    plt.ylim(0.0,1.0)
    ...
    for j in range(2**(n_oscillators-1)):
        ...
        plt.plot(tlist/K,df["Prob_conf"][str(j)]+df["Prob_conf"][str(2**n_oscillators-1-j)],label=r"$\left |"+string+r"\right>$ + inv.")
    ...
    plt.xlim(0.1,tlist[np.where(-f(tlist)<=10,True,False)][-1])
    plt.tight_layout()
    plt.ylabel("Probability",fontsize=20)
    plt.xlabel("tK",fontsize=20)
