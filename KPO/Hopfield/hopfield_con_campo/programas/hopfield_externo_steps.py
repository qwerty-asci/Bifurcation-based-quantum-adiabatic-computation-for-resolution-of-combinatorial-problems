...
try:
    shm1 = shared_memory.SharedMemory(name="Matrix1",create=True, size=M1.nbytes)
    shm2 = shared_memory.SharedMemory(name="Matrix2",create=True, size=M1.nbytes)
    ...
    for j in tqdm(range(2**(n_oscillators))):
        ...
        if(j%2==0):
            ...
            memory.append(shared_memory.SharedMemory(name="matrix"+str(matrixcont),create=True,size=proyect.nbytes))
...
def medidas_bloque2(fila,estado,archivo):
    ...
    for j in range(2**(n_oscillators-1)):
        share.append(shared_memory.SharedMemory(name="matrix"+str(j)))
    ...
    shared1 = shared_memory.SharedMemory(name="Matrix1")
    shared2 = shared_memory.SharedMemory(name="Matrix2")
...
try:
    ...
    for j in tqdm(range(int(samples/bloques))):
        ...
        trabajos.append(ex.submit(medidas_bloque2,[i for i in range(contador*bloques,(contador+1)*bloques)],result.states,resultados))
        contador+=1
