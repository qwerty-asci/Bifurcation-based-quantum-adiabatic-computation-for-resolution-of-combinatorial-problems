...
def trayectoria(estado_inicial,lista_de_hamiltonianos,operadores_de_salto,bloques,tlist,id_proceso,hilos_medidas,nombres,medidas_bloque2,id_fichero,seed):
    ...
    nombre_fichero_resultados="osciladores_disipativos_motecarlo_id_"+str(id_fichero)+"_n_osc_"+str(n_oscillators)+"_N_"+str(N)+"_m_"+str(m)+"_delta_"+str(delta[0])+"_gammam_"+str(gammam)+"_gamma_"+str(gamma)+"_eta_"+str(etaf)+"_theta_"+str(theta)+"_n_"+str(n)+"_xi_"+str(xi)+"_tau_"+str(tau)+"_J_"+str(J[0,1])+".csv"
...
try:
    ...
    for j in range(numero_de_trayectorias):
        trabajos.append(ex.submit(trayectoria,psi0,[H0,[Ht,f]],jump,bloques,tlist,rng.randint(0,10000000000),2,nombres,medidas_bloque2,j,rng.randint(0,10000000000)))
    ...
    with open("tiempo.dat","a") as archivo:
        archivo.write(str(N**n_oscillators)+" "+str(N)+" "+str(n_oscillators)+" "+str(t2-t1)+" "+str(tracemalloc.get_traced_memory()[0]/1024.0**2)+" "+str(tracemalloc.get_traced_memory()[1]/1024.0**2)+"\n")
...
try:
    nombre1="osciladores_disipativos_motecarlo_id_"
    ...
    datos_e=datos.std(axis=0)
    
    resultados.iloc[:,:2**n_oscillators]=datos_m
    resultados.iloc[:,2**n_oscillators:]=datos_e

    
    resultados.to_csv("osciladores_disipativos_motecarlo_n_osc_"+str(n_oscillators)+"_N_"+str(N)+"_m_"+str(m)+"_delta_"+str(delta[0])+"_gammam_"+str(gammam)+"_gamma_"+str(gamma)+"_eta_"+str(etaf)+"_theta_"+str(theta)+"_n_"+str(n)+"_xi_"+str(xi)+"_tau_"+str(tau)+"_J_"+str(J[0,1])+".csv")
