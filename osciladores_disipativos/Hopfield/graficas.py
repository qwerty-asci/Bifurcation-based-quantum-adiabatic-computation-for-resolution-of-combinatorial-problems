...
name="osciladores_disipativos_motecarlo_n_"
...
for nombre in os.listdir("resultados"):
    ...
    plt.title("Success probability of a Hopfield configuration",fontsize=16)
    ...
    if(True):
        for j in range(2**(n_oscillators-1)):
            ...
            plt.errorbar(tlist[::4],df["Prob_conf"][str(j)][::4]+df["Prob_conf"][str(2**n_oscillators-j-1)][::4],yerr=5*np.sqrt(df["Error"][str(j)][::4]**2+df["Error"][str(2**n_oscillators-j-1)][::4]**2),label=r"$\left|"+string+r"\right>$ + inv.",ecolor="red",elinewidth=0.5,capsize=2,linewidth=2)
    else:
